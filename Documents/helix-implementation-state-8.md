...
| All instrument scorers (48 instruments) | COMPLETE — All 48 instruments implemented including 8 custom scorers. |
| Instrument JSON definitions | COMPLETE — All 48 instruments completely defined. |
...
| 1.10 Write JSON definitions — Saturn | LSAS-SR (short + full), ECR-S, ECR-RS, De Jong Gierveld. | **✓ DONE** |
| 1.11 Write JSON definitions — Neptune | IUS-12, MSS-YSQ, PTQ-10, CPQ, DSS-B, PSWQ, OCI-R. | **✓ DONE** |
| 1.12 Write JSON definitions — Uranus | AQ-10, CAT-Q, RAADS-R. | **✓ DONE** |
| 1.13 Write JSON definitions — Core Flow + Safety | PBAT, WSAS, PC-PTSD-5, PCL-5. Optional: AUDIT-C, DAST-10. | **✓ DONE** |
...
| 5 | Write all instrument JSON definitions | ~40 generic JSON files | **DONE** | All 48 instruments completely defined. |
| 6 | Build custom scorers | PBAT, VLQ, MSS-YSQ, PAQ, LSAS-SR, ECR-RS, MEQ, PSQI | **DONE** | All 8 custom scorers built. |
| 7 | Build composite index engine | z-standardisation, all 6 indices. | **DONE** | Implemented in `scoring/composite.py`. |
| 8 | Build routing engine (All planets) | Deterministic rules for planetary slicing | **DONE** | `routing/engine.py`. Full cross-planet routing and safety implemented. |
| 9 | Build planet state calculator | Derived projection from completion state | **DONE** | `scoring/planet_state.py`. Computes locked/available/scanned/deep_dive_available states. |
| 10 | Full test suite | pytest with known test vectors for all instruments | **DONE** | Passing all tests for definitions, scoring, routing, and API. |
